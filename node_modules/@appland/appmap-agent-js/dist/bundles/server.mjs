...
import { URL as URL$c, pathToFileURL, fileURLToPath, URLSearchParams } from 'node:url';
...
const toRelativeUrl = (
  ...
  const { protocol, host, pathname, search, hash } = new URL$c(url);
  const {
...
const self_package = parseJSON$7(
  await readFile$1(
    new URL$b(toAbsoluteUrl("package.json", self_directory)),
    "utf8",
  ),
);
...
const { Boolean: Boolean$3, Error: Error$9, Number: Number$1, undefined: undefined$9 } = globalThis;
...
const { URL: URL$a } = globalThis;

const openLogFile = (specifier) => {
  if (typeof specifier === "number") {
    return specifier;
  } else if (typeof specifier === "string") {
    return openSync(new URL$a(specifier), "w");
  } else {
...
const readPackage = (url) => {
  try {
    return {
      url,
      content: readFileSync(new URL$9("package.json", url), "utf8"),
    };
  } catch (error) {
...
const { URL: URL$8 } = globalThis;

const spawn = (exec, argv, url) => {
  const result = spawnSync(exec, argv, {
    cwd: fileURLToPath(new URL$8(".", url)),
    encoding: "utf8",
    timeout: 10000,
...
const { URL: URL$7, parseInt: parseInt$3 } = globalThis;
...
const readRepository = (url) => {
  try {
    return readdirSync(new URL$7(url));
  } catch (error) {
...
} = await import(new URL$6("dist/schema.mjs", self_directory));
...
const loadJsonAsync = async (url) =>
  parseJson(await readFile$1(new URL$5(url), "utf8"));

const loadYamlAsync = async (url) =>
  parse$1(await readFile$1(new URL$5(url), "utf8"));
...
const loadPackageJsonAsync = async (directory) => {
  try {
    return parseJSON$5(
      await readFile$1(
        new URL$4(toAbsoluteUrl("package.json", directory)),
        "utf8",
      ),
    );
  } catch (error) {
...
const resolvePresetSpecifier = (specifier, root) => {
  if (specifier.startsWith("./") || specifier.startsWith("../")) {
    ...
    const { resolve } = createRequire(new URL$4(root));
...
const splitSpecifier = (specifier) => {
  if (specifier.startsWith("./") || specifier.startsWith("../")) {
    ...
    const { protocol, host, pathname, search, hash } = new URL$c(specifier);
    assert(
      pathname.startsWith("/"),
...
const makeFailure = ({ message, location }, { base }) => {
  ...
  const relative = toRelativeUrl(new URL$3(url, base), base);
  return { message, location: [relative, line].filter(Boolean$1).join(":") };
};
...
const resolveHostPath = ({ name, port }, path) =>
  new URL$c(path, `http://${name}:${String$3(port)}`).href;

const parseHost = (host_string) => {
  const { hostname: name, port: port_string } = new URL$c(
    `http://${host_string}`,
  );
  return {
    name,
...
const makeHashUrl = (url, { start, end }) => {
  const url_object = new URL$c(url);
...
const recorder_browser_content = readFileSync(
  new URL$c("dist/bundles/recorder-browser.mjs", self_directory),
  "utf8",
);
