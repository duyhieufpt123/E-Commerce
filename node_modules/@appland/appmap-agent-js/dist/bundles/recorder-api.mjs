...
import { URL as URL$7, pathToFileURL, fileURLToPath, URLSearchParams } from 'node:url';
...
const toRelativeUrl = (
  ...
  const { protocol, host, pathname, search, hash } = new URL$7(url);
  const {
...
const { Boolean: Boolean$3, Error: Error$7, Number: Number$1, undefined: undefined$d } = globalThis;
...
} = await import(new URL$4("dist/schema.mjs", self_directory));
...
const { Error: Error$1, decodeURIComponent, URL: URL$3 } = globalThis;
...
const requirePeerDependency = (specifier, { directory, strict }) => {
  const require = createRequire(new URL$2(directory));
  try {
...
const splitSpecifier = (specifier) => {
  if (specifier.startsWith("./") || specifier.startsWith("../")) {
    ...
    const { protocol, host, pathname, search, hash } = new URL$7(specifier);
    assert(
      pathname.startsWith("/"),
...
const makeFailure = ({ message, location }, { base }) => {
  ...
  const relative = toRelativeUrl(new URL$1(url, base), base);
